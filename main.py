...
import random #This is a bit weird, but it works
...
def main():
	try:
		...
		response = session.post(SURVEY_URL, headers=headers, data=data, proxies=proxy)
		# if this string is in the reponse, the request was successful
		if("Dein Jugendwort ist jetzt bei uns aufgenommen." in response.text):
